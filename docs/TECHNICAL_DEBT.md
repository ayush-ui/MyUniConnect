...
| DEBT-018 | Mobile / Branding | **Android adaptive icon uses the full-bleed mark as `foregroundImage`** (`assets/brand/mark/unisync-mark-1024.png`) with a teal background. The mark has no built-in safe-zone padding, so circular/squircle masks may clip its edges; it is also full-colour (teal elements can blend into the teal bg). | Produce a padded, single-colour (cream) adaptive foreground at the ~66% safe zone before store submission. |
| DEBT-019 | Mobile / Branding | **Animated splash/loader not implemented natively.** `assets/brand/unisync-loader.html` is the designed "Quad builds itself" animation, kept as a reference artifact only. The app currently shows a **static** splash (mark on cream via `expo-splash-screen`). | Translate the loader to a native animated splash (Reanimated/Lottie) or an animated first-paint screen — polish, post-launch. |
| DEBT-020 | Marketplace / Identity | **Listing payload has no seller identity.** `Listing` exposes only `sellerId`, so the listing-detail trust badge renders a **static** "Verified student" (correct — only verified students can post) **without the university name**, and the grid-card verified badge (Figma `94:2`) is not shown. | Add a `seller { firstName, lastInitial, university, isVerifiedStudent }` projection to listing list/detail responses; then show "✓ Verified student · <Uni>" on detail and a compact badge on grid cards. |
| DEBT-021 | Branding / Repo | **Legacy "MyUniConnect" identifiers remain** after the UniSync rename. App-facing surfaces are done (Expo name/icon/splash/scheme/bundle ids, verification email, Swagger title). Still legacy: repo directory name, workspace package `@myuniconnect/shared` (+ its imports), `apps/*/.env.example` (`EMAIL_FROM`, `S3_BUCKET`), and `apps/mobile/docs/EPIC1_APIS.json` title. Bundle ids changed to `com.unisync.app` (no store identity existed yet). | Coordinated internal rename in a dedicated PR (workspace pkg + imports, env templates, repo dir) — low urgency, pre-launch. |
