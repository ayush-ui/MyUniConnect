...
| DEBT-013 | Mobile / Testing | No screen- or component-level tests. Only API layers, `AuthContext`, `useMarketplace`, and storage are covered (66 tests). `login.tsx`, `register.tsx`, marketplace screens, and `components/ui/*` are untested; no E2E layer. | Phase 0 of `docs/EXECUTION_PLAN.md` (backfill before new epics) |
| DEBT-012 | Auth | `POST /auth/resend-verification` has no rate limiting. EPIC-001 UC-1.3 specifies max 3 requests/hour per email. | Add NestJS throttler (per-email key) when auth hardening lands — tied to INFRA-005 (rate-limit persistence) |
